...
#       Columns are [indicator] [Ndiagnosis + Nmedication + Nlab features]
  ...
  if ( typeString == "test" ) {
    ...
    flatDataset$Gender <- as.integer(flatDataset$Gender)
  }
  else {
  ...
  if ( typeString == "test" ) {
    ...
    flatDataset <- addTranscriptVariables(con, "test", flatDataset) 
  } else {
    ...
    flatDataset <- addTranscriptVariables(con, "training", flatDataset)
  }
...
clean_up_outliers <- function(v, minValue, maxValue, replaceWith=NA) {
  ifelse (v < minValue | v > maxValue, replaceWith, v)
}
...
addTranscriptVariables <- function(con, typeString, flatDataset) {  
  ...
  if ( typeString == "test" ) {
    tableToRead <- dbGetQuery(con, "SELECT * FROM test_transcript ORDER BY VisitYear")
  }
  else {
    tableToRead <- dbGetQuery(con, "SELECT * FROM training_transcript ORDER BY VisitYear")
  }  
  
  weights <- aggregate(tableToRead[, c("Weight")], list(PatientGuid=tableToRead$PatientGuid), get_first_value)
  bmis <- aggregate(tableToRead[, c("BMI")], list(PatientGuid=tableToRead$PatientGuid), get_first_value)
  weights$x <- clean_up_outliers(weights$x, 0.1, 1500)
  bmis$x <- clean_up_outliers(bmis$x, 0.1, 50)
  colnames(weights) <- c("PatientGuid", "Weight")
  colnames(bmis) <- c("PatientGuid", "BMI")
  
  flatDataset <- merge(flatDataset, weights, by="PatientGuid")
  flatDataset <- merge(flatDataset, bmis, by="PatientGuid")
